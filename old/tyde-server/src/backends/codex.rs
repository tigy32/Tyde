...
use tyde_protocol::protocol::{
    ChatEvent, ChatMessage, ContextBreakdown, ImageData, MessageSender, ModelInfo, ModelsListData,
    ModelsListEntry, OperationCancelledData, ReasoningData, SessionMetadata, SessionStartedData,
    SessionsListData, StreamEndData, StreamStartData, StreamTextDeltaData, SubprocessExitData,
    Task, TaskList, TaskStatus, TokenUsage, ToolExecutionCompletedData, ToolExecutionResult,
    ToolRequest, ToolRequestType, ToolUseData,
};

use crate::agent::CommandExecutor;
use crate::backends::claude::{SubAgentEmitter, SubAgentHandle};
...
impl CommandExecutor for CodexCommandHandle {
    fn execute(
        &self,
        command: SessionCommand,
    ) -> Pin<Box<dyn Future<Output = Result<(), String>> + Send + '_>> {
        Box::pin(CodexCommandHandle::execute(self, command))
    }
}
...
impl CodexSession {
    ...
    async fn spawn_with_mode(
        ...
        inner.emit_event(ChatEvent::SessionStarted(SessionStartedData { session_id }));
...
pub async fn query_account_rate_limits(transport: BackendTransport) -> Result<Value, String> {
    let _ = transport;
    let (rpc, _inbound_rx) = CodexRpc::spawn(&BackendTransport::Local, &[], None).await?;
...
struct CodexState {
    ...
    next_phase_ordinal: u64,
    token_usage_by_turn: HashMap<String, CodexTokenUsage>,
...
impl CodexInner {
        ...
        match command {
            ...
            SessionCommand::UpdateSettings {
                ...
                if let Some(model_value) = settings.model {
                    state.model = model_value.and_then(|model| {
                ...
                if let Some(effort_value) = settings.reasoning_effort {
                    state.reasoning_effort =
                        effort_value.as_deref().and_then(normalize_reasoning_effort);
                }
            ...
            for thread in page {
                if let Some(metadata) = codex_thread_to_session_metadata(&thread) {
                    if !workspace_root.is_empty() {
                        let session_root = metadata.workspace_root.as_deref().unwrap_or_default();
    ...
    async fn resume_session(&self, session_id: String) -> Result<(), String> {
        ...
        let mut state = self.state.lock().await;
        state.context_bytes_total = restored_bytes;
    ...
    async fn list_models(&self) -> Result<(), String> {
        ...
        let models: Vec<ModelsListEntry> = raw_models
            .iter()
            .filter_map(|m| {
                let id = m
                    .get("model")
                    ...
                    "agentMessage" => {
                        ...
                        self.emit_event(ChatEvent::MessageAdded(ChatMessage {
                            ...
                            sender: MessageSender::Assistant {
                                agent: CODEX_AGENT_NAME.to_string(),
                            },
                            content: text,
    ...
    async fn handle_inbound(&self, inbound: CodexInbound) {
        match inbound {
            ...
            CodexInbound::Closed { exit_code } => {
                ...
                .await;
                self.emit_event(ChatEvent::SubprocessExit(SubprocessExitData { exit_code }));
            }
        ...
        match method {
            "turn/started" => {
                    ...
                    .to_string();
                {
                    ...
                    state.pending_user_input_bytes = 0;
                    state.context_bytes_total =
                        state.context_bytes_total.saturating_add(pending_user_input);
                    let history_bytes = state.context_bytes_total;
            ...
            "item/agentMessage/delta" => {
                ...
                let (phase_to_close, start_event, phase_id) = {
                    let mut state = self.state.lock().await;
                    let (phase_to_close, start_event, phase_id) =
                        prepare_phase_for_assistant_output(&mut state, Some(message_id.as_str()));
                    if let Some(phase) = state.active_phase.as_mut() {
                        phase.text.push_str(&delta);
                    }
                    (phase_to_close, start_event, phase_id)
                };
                if let Some(phase) = phase_to_close {
                    self.emit_phase_end(phase);
                }
                if let Some(event) = start_event {
                    self.emit_event(event);
                }
                self.emit_event(ChatEvent::StreamDelta(StreamTextDeltaData {
                    message_id: Some(phase_id),
                    text: delta,
                }));
            }
        ...
        match method {
            "turn/started" => {
                let turn_id = params
                    ...
                    .unwrap_or("turn")
                    .to_string();
                emit_event_to(event_tx, ChatEvent::TypingStatusChanged(true));
                emit_event_to(
                    event_tx,
            ...
            "error" => {
                let message = params
                    ...
                    .to_string();
                emit_event_to(event_tx, ChatEvent::Error(message));
                emit_event_to(event_tx, ChatEvent::TypingStatusChanged(false));
            }
            _ => {}
        }
    }
            ...
            "collabToolCall" | "collabAgentToolCall" | "mcpToolCall" | "dynamicToolCall" => {
                ...
                emit_event_to(
                    ...
                    ChatEvent::ToolRequest(ToolRequest {
                        tool_call_id: item_id,
                        tool_name,
                        tool_type: ToolRequestType::Other { args: item.clone() },
                    }),
                );
            }
            _ => {}
    ...
    async fn emit_reasoning_delta(&self, params: &Value, delta: String) {
        ...
        let Some((phase_to_close, start_event, event)) = event else {
            return;
        };
        if let Some(phase) = phase_to_close {
            self.emit_phase_end(phase);
    ...
    fn emit_phase_end(&self, phase: CodexPhaseEmission) {
        tracing::trace!(
            "emitting codex phase end: turn_id={} phase_id={}",
            phase.turn_id,
            phase.phase_id
        );
        self.emit_event(ChatEvent::StreamEnd(StreamEndData {
            message: ChatMessage {
                ...
                content: phase.text,
                reasoning: (!phase.reasoning.trim().is_empty()).then_some(ReasoningData {
                ...
                model_info: Some(ModelInfo { model: phase.model }),
                token_usage: phase.token_usage,
                context_breakdown: phase.context_breakdown,
                images: Some(Vec::new()),
            },
        }));
    }

    async fn record_and_emit_tool_execution_completed(
        &self,
        tool_call_id: &str,
        tool_name: &str,
        success: bool,
        ...
        {
            let mut state = self.state.lock().await;
            mark_tool_completion_for_active_phase(&mut state, tool_call_id);
        }
            ...
            "item/tool/call" => {
                ...
                self.emit_tool_execution_completed(
                    ...
                    ToolExecutionResult::Error {
                        short_message:
                            "Dynamic client tool calls are not yet supported in Tyde."
                                .to_string(),
                        detailed_message:
                            "Codex requested a client-side dynamic tool call that Tyde has not implemented yet."
                                .to_string(),
                    },
    ...
    async fn handle_item_started(&self, params: &Value) {
        ...
        match item_type {
            "commandExecution" => {
                ...
                let start_event = {
                    let mut state = self.state.lock().await;
                    let start_event = prepare_phase_for_tool_output(&mut state);
                    register_tool_call_for_active_phase(
                        &mut state,
                        &item_id,
                        "run_command",
            ...
            "fileChange" => {
                ...
                let start_event = {
                    ...
                    let start_event = prepare_phase_for_tool_output(&mut state);
                    for (change, call_id) in file_changes.iter().zip(call_ids.iter()) {
                        register_tool_call_for_active_phase(
                            &mut state,
                            call_id,
                            "modify_file",
        ...
        match item_type {
            "agentMessage" => {
                let text = item
                    ...
                    .unwrap_or_default()
                    .to_string();
                let item_reasoning = extract_codex_item_reasoning(item);
                let (phase_to_close, start_event, finalized_phase) = {
                    let mut state = self.state.lock().await;
                    let (phase_to_close, start_event, _) =
                        prepare_phase_for_assistant_output(&mut state, Some(item_id.as_str()));
                    if let Some(phase) = state.active_phase.as_mut() {
                        if !text.is_empty() {
                            phase.text = text.clone();
                        }
                        if phase.reasoning.trim().is_empty() {
                            if let Some(reasoning) = item_reasoning.as_ref() {
                                phase.reasoning = reasoning.clone();
                            }
                        }
                        phase.text_final = true;
                    }
                    state.context_bytes_total =
                        state.context_bytes_total.saturating_add(text.len() as u64);
                    let active_phase_turn_id = state
                        .active_phase
                        .as_ref()
                        .map(|phase| phase.turn_id.clone());
                    let turn_still_active = active_phase_turn_id
                        .as_deref()
                        .is_some_and(|turn_id| state.active_turn_id.as_deref() == Some(turn_id));
                    let finalized_phase = if turn_still_active {
                        None
                    } else {
                        take_active_phase_emission(&mut state, true)
                    };
                    if let Some(phase) = finalized_phase.as_ref() {
                        state.token_usage_by_turn.remove(&phase.turn_id);
                        state.turn_context_by_turn.remove(&phase.turn_id);
                    }
                    (phase_to_close, start_event, finalized_phase)
                };
                if let Some(phase) = phase_to_close {
                    self.emit_phase_end(phase);
                }
                ...
                if let Some(phase) = finalized_phase {
                    self.emit_phase_end(phase);
                }
            }
            "userMessage" => {
            ...
            "reasoning" => {
                ...
                self.emit_reasoning_delta(&json!({ "itemId": item_id }), reasoning_text)
                    .await;
            }
            "commandExecution" => {
                ...
                let output = item
                    ...
                    .to_string();
                let success = exit_code == 0;
                self.record_and_emit_tool_execution_completed(
                    &item_id,
                    "run_command",
                    success,
                    ToolExecutionResult::RunCommand {
    ...
    async fn handle_turn_completed(&self, params: &Value) {
        ...
        let finalized_phase = {
            ...
            state.active_turn_id = None;
            let finalized_phase = if completed_turn_id.as_ref().is_some_and(|turn_id| {
                state
                    .active_phase
                    .as_ref()
                    .is_some_and(|phase| phase.turn_id == *turn_id)
            }) {
                take_active_phase_emission(&mut state, true)
            } else {
                None
    ...
    fn emit_user_message_added(&self, content: &str, images: Option<&[ImageAttachment]>) {
        ...
        self.emit_event(ChatEvent::MessageAdded(ChatMessage {
            ...
            token_usage: None,
            context_breakdown: None,
            images: Some(image_payload),
        }));
    }

    fn emit_event(&self, event: ChatEvent) {
        if let Err(error) = self.event_tx.send(event) {
            tracing::error!("Codex event channel closed: {error}");
        }
    }
}

fn emit_event_to(event_tx: &mpsc::UnboundedSender<ChatEvent>, event: ChatEvent) {
    if let Err(error) = event_tx.send(event) {
        tracing::error!("Codex sub-agent event channel closed: {error}");
    }
}
...
fn find_subagent_event_tx_for_thread(
    state: &CodexState,
    thread_id: &str,
) -> Option<mpsc::UnboundedSender<ChatEvent>> {
...
fn codex_plan_update_event_from_params(params: &Value) -> Option<ChatEvent> {
    let title = params
        .get("explanation")
        .and_then(Value::as_str)
        .filter(|s| !s.trim().is_empty())
    ...
    Some(ChatEvent::TaskUpdate(TaskList { title, tasks }))
}

fn codex_thread_to_session_metadata(thread: &Value) -> Option<SessionMetadata> {
...
fn next_codex_phase_id(state: &mut CodexState, phase_id_hint: Option<&str>) -> String {
    if let Some(phase_id) = phase_id_hint
        .map(str::trim)
        .filter(|value| !value.is_empty())
    {
        return phase_id.to_string();
    }
    ...
    state.next_phase_ordinal = state.next_phase_ordinal.saturating_add(1);
    let turn_id = state
        .active_turn_id
        .as_deref()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or("turn");
    format!("{turn_id}-phase-{}", state.next_phase_ordinal)
}

fn codex_stream_start_event(state: &CodexState, phase_id: &str) -> ChatEvent {
    ChatEvent::StreamStart(StreamStartData {
        message_id: Some(phase_id.to_string()),
        agent: CODEX_AGENT_NAME.to_string(),
        model: Some(state.model.clone().unwrap_or_else(|| "codex".to_string())),
    })
}

fn phase_has_unsettled_tools(phase: &ActivePhaseState) -> bool {
    phase
        .requested_tool_ids
        .iter()
        .any(|tool_call_id| !phase.completed_tool_ids.contains(tool_call_id))
}

fn take_active_phase_emission(
    state: &mut CodexState,
    allow_partial: bool,
) -> Option<CodexPhaseEmission> {
    let can_take = state
        .active_phase
        .as_ref()
        .is_some_and(|phase| allow_partial || phase.text_final);
    if !can_take {
        return None;
    }

    let phase = state.active_phase.take()?;
    if phase_has_unsettled_tools(&phase) {
        tracing::error!(
            "[CODEX PROTOCOL VIOLATION] assistant phase closed before all tool completions arrived: phase_id={} outstanding_tools={:?}",
            phase.phase_id,
            phase
                .requested_tool_ids
                .difference(&phase.completed_tool_ids)
                .cloned()
                .collect::<Vec<_>>()
        );
    }

    let has_payload = !phase.text.trim().is_empty()
        || !phase.reasoning.trim().is_empty()
    ...
    let turn_context = state
        ...
        .cloned()
        .unwrap_or_default();
    let context_breakdown = Some(estimate_context_breakdown(
        state.token_usage_by_turn.get(&phase.turn_id),
        &turn_context,
        Some(&model),
    ));

    Some(CodexPhaseEmission {
        turn_id: phase.turn_id,
...
fn prepare_phase_for_assistant_output(
    state: &mut CodexState,
    phase_id_hint: Option<&str>,
) -> (Option<CodexPhaseEmission>, Option<ChatEvent>, String) {
    let phase_to_close = if state
        .active_phase
        .as_ref()
        .is_some_and(|phase| phase.text_final)
    {
        take_active_phase_emission(state, false)
    } else {
        None
    };

    if let Some(phase) = state.active_phase.as_ref() {
        return (phase_to_close, None, phase.phase_id.clone());
    }

    let phase_id = next_codex_phase_id(state, phase_id_hint);
    let turn_id = state
        .active_turn_id
        .clone()
        .unwrap_or_else(|| "turn".to_string());
    state.active_phase = Some(ActivePhaseState {
        turn_id,
        phase_id: phase_id.clone(),
        ..ActivePhaseState::default()
    });
    let start_event = Some(codex_stream_start_event(state, &phase_id));
    (phase_to_close, start_event, phase_id)
}

fn prepare_phase_for_tool_output(state: &mut CodexState) -> Option<ChatEvent> {
    if state.active_phase.is_some() {
        return None;
    }

    let phase_id = next_codex_phase_id(state, None);
    let turn_id = state
        .active_turn_id
        .clone()
        .unwrap_or_else(|| "turn".to_string());
    ...
    Some(codex_stream_start_event(state, &phase_id))
}

fn register_tool_call_for_active_phase(
    state: &mut CodexState,
    tool_call_id: &str,
    tool_name: &str,
    arguments: Value,
) {
    let Some(phase) = state.active_phase.as_mut() else {
        tracing::error!(
            "[CODEX PROTOCOL VIOLATION] tried to register tool request without an active phase: tool_call_id={tool_call_id} tool_name={tool_name}"
        );
        return;
    };

    if !phase.declared_tool_ids.insert(tool_call_id.to_string()) {
        phase.requested_tool_ids.insert(tool_call_id.to_string());
        return;
    }

    phase.requested_tool_ids.insert(tool_call_id.to_string());
    phase.tool_calls.push(ToolUseData {
...
fn mark_tool_completion_for_active_phase(state: &mut CodexState, tool_call_id: &str) {
    let Some(phase) = state.active_phase.as_mut() else {
        tracing::error!(
            "[CODEX PROTOCOL VIOLATION] received tool completion without an active phase: tool_call_id={tool_call_id}"
        );
        return;
    };

    if !phase.requested_tool_ids.contains(tool_call_id) {
        tracing::error!(
            "[CODEX PROTOCOL VIOLATION] received tool completion without a matching request in the active phase: phase_id={} tool_call_id={tool_call_id}",
            phase.phase_id
        );
        return;
    }

    if !phase.completed_tool_ids.insert(tool_call_id.to_string()) {
        tracing::error!(
            "[CODEX PROTOCOL VIOLATION] duplicate tool completion in active phase: phase_id={} tool_call_id={tool_call_id}",
            phase.phase_id
        );
    }
}
...
fn apply_reasoning_delta_to_state(
    ...
    params: &Value,
    delta: &str,
) -> Option<(Option<CodexPhaseEmission>, Option<ChatEvent>, ChatEvent)> {
    if delta.is_empty() {
        return None;
    }
    ...
    let (phase_to_close, start_event, phase_id) = prepare_phase_for_assistant_output(state, hint);

    let delta = if let Some(phase) = state.active_phase.as_ref() {
        merge_reasoning_delta(&phase.reasoning, delta)
    } else {
        delta.to_string()
    };
    if delta.is_empty() {
    ...
    if let Some(phase) = state.active_phase.as_mut() {
        phase.reasoning.push_str(&delta);
    }
...
fn map_plan_status(status: &str) -> TaskStatus {
    match status {
...
fn extract_turn_token_usage(
    params: &Value,
    model_hint: Option<&str>,
) -> Option<(String, CodexTokenUsage)> {
...
fn estimate_context_breakdown(
    ...
    let base_input_tokens = token_usage
        .map(|usage| usage.token_usage.input_tokens)
        .unwrap_or(0);
    let cached_prompt_tokens = token_usage
        .and_then(|usage| usage.token_usage.cached_prompt_tokens)
        .unwrap_or(0);
    let cache_creation_input_tokens = token_usage
        .and_then(|usage| usage.token_usage.cache_creation_input_tokens)
        .unwrap_or(0);
    ...
    let reasoning_from_tokens = token_usage
        .and_then(|usage| usage.token_usage.reasoning_tokens)
        .unwrap_or(0)
    ...
    let history_est = turn_context.history_bytes;
    ...
    let history_bytes = std::cmp::min(history_est, remaining);
    remaining = remaining.saturating_sub(history_bytes);
...
mod tests {
    ...
    fn test_codex_state() -> CodexState {
        CodexState {
            ...
            active_phase: Some(ActivePhaseState {
                ...
                phase_id: "msg-seed".to_string(),
                text: String::new(),
                reasoning: String::new(),
                tool_calls: Vec::new(),
                declared_tool_ids: HashSet::new(),
                requested_tool_ids: HashSet::new(),
    ...
    fn test_codex_inner() -> (Arc<CodexInner>, mpsc::UnboundedReceiver<ChatEvent>) {
    ...
    #[tokio::test]
    async fn codex_declares_tool_calls_on_stream_end_for_tool_first_phase() {
        let (inner, mut event_rx) = test_codex_inner();

        inner
            .handle_notification("turn/started", &json!({ "turn": { "id": "turn-1" } }))
            .await;
        inner
            .handle_notification(
                "item/started",
        ...
        assert_eq!(
            tool_calls[0].get("name").and_then(Value::as_str),
            Some("run_command")
        );
        assert_eq!(
            tool_calls[0]
    ...
    #[tokio::test]
    async fn codex_splits_two_assistant_phases_within_one_turn() {
        let (inner, mut event_rx) = test_codex_inner();
    ...
    impl SubAgentEmitter for RecordingSubAgentEmitter {
        ...
        ) -> std::pin::Pin<Box<dyn std::future::Future<Output = SubAgentHandle> + Send + '_>>
        {
            Box::pin(async move {
                ...
                SubAgentHandle { agent_id, event_tx }
            })
        }
        ...
        fn on_subagent_completed(
            ...
            success: bool,
            final_response: Option<String>,
            _event_tx: tokio::sync::mpsc::UnboundedSender<ChatEvent>,
    ...
    fn live_codex_spawn_agent_round_trip_emits_subagent_callbacks() {
        ...
        rt.block_on(async {
            ...
            let (session, mut event_rx) = CodexSession::spawn(
                ...
                &[],
                None,
                Some(emitter.clone() as Arc<dyn SubAgentEmitter>),
            )
            .await
            .expect("spawn codex session");
            live_test_log("CodexSession spawned with sub-agent emitter");
...
If you skip spawn_agent or wait_agent, this test fails."#;
    ...
    fn subagent_thread_notifications_route_to_subagent_channel_not_parent() {
        ...
        rt.block_on(async {
            ...
            assert!(
                ...
                "sub-agent thread notifications should not emit into parent agent: {parent_events:?}"
            );
    ...
    fn apply_reasoning_delta_to_state_emits_event_and_updates_state() {
        ...
        let (_phase_to_close, start_event, event) =
        ...
        assert_eq!(event["data"]["message_id"], json!("msg-seed"));
        assert_eq!(event["data"]["text"], json!("Inspecting constraints."));

        let phase = state.active_phase.as_ref().expect("active phase");
        assert_eq!(phase.phase_id, "msg-seed");
        assert_eq!(phase.reasoning, "Inspecting constraints.");
    ...
    fn apply_reasoning_delta_to_state_ignores_duplicate_delta_payloads() {
        ...
        let phase = state.active_phase.as_ref().expect("active phase");
        assert_eq!(phase.reasoning, "Planning targeted web search");
    }

...
use tyde_protocol::protocol::{
    ...
    ReasoningData, SessionMetadata, SessionSettingsData, SessionStartedData, SessionsListData,
    StreamEndData, StreamStartData, StreamTextDeltaData, Task, TaskList, TaskStatus, TokenUsage,
...
use crate::backends::types::{
    AgentIdentity, SessionCommand, StartupMcpServer, StartupMcpTransport,
};
...
impl CommandExecutor for ClaudeCommandHandle {
    fn execute(
        &self,
        command: SessionCommand,
    ) -> Pin<Box<dyn Future<Output = Result<(), String>> + Send + '_>> {
        Box::pin(ClaudeCommandHandle::execute(self, command))
    }
}
...
impl ClaudeSession {
    ...
    async fn spawn_with_mode(
        ...
        let workspace_root = pick_workspace_root(workspace_roots)?;
...
impl ClaudeInner {
    async fn execute_arc(this: Arc<Self>, command: SessionCommand) -> Result<(), String> {
        match command {
            ...
            SessionCommand::ListProfiles => {
                this.emit_event(ChatEvent::ProfilesList(ProfilesListData {
                    profiles: Vec::new(),
                }));
                Ok(())
            }
            ...
            SessionCommand::GetModuleSchemas => {
                this.emit_event(ChatEvent::ModuleSchemas(ModuleSchemasData {
                    schemas: Vec::new(),
                }));
                Ok(())
            }
            SessionCommand::ListModels => {
                this.emit_event(ChatEvent::ModelsList(ModelsListData {
            ...
            SessionCommand::UpdateSettings {
                ...
                if let Some(effort_value) = settings.effort.or(settings.reasoning_effort) {
                    state.effort = effort_value
                        .and_then(|effort| normalize_claude_effort(&Value::String(effort)));
                }

                if let Some(permission_mode_value) =
                    settings.permission_mode.or(settings.approval_policy)
                {
                    state.permission_mode = permission_mode_value.and_then(|permission_mode| {
                        normalize_claude_permission_mode(&Value::String(permission_mode))
                    });
                }
                this.emit_settings().await;
                Ok(())
    ...
    async fn start_turn(self: Arc<Self>, message: String, images: Option<Vec<ImageAttachment>>) {
        ...
        let (
            ...
            state.context_bytes_total = state.context_bytes_total.saturating_add(input_bytes);
            ...
            match outcome {
                TurnOutcome::Completed {
                    ...
                    if let Some(phase) = take_phase_emission(&mut summary) {
                        ...
                        let tool_calls = phase
                            .tool_calls
                            .iter()
                            .map(tool_use_data_from_claude_tool_call)
                            .collect::<Vec<_>>();
                        ...
                        if !text.is_empty() {
                            self.add_context_bytes(text.len() as u64).await;
                        }
                ...
                TurnOutcome::Cancelled {
                    ...
                    if let Some(phase) = take_phase_emission(&mut summary) {
                        ...
                        if !partial.is_empty() {
                            self.add_context_bytes(partial.len() as u64).await;
                        }
                        self.emit_stream_end(
                            partial,
                            selected_model,
    ...
    async fn add_context_bytes(&self, bytes: u64) {
        if bytes == 0 {
            return;
        }
        let mut state = self.state.lock().await;
        state.context_bytes_total = state.context_bytes_total.saturating_add(bytes);
    }
    ...
    async fn resume_session(&self, session_id: String) -> Result<(), String> {
        ...
        for item in replay.items {
            match item {
                ClaudeHistoryReplayItem::Message(message) => {
                    self.emit_event(ChatEvent::MessageAdded(message));
                }
        ...
        state.context_bytes_total = replay.context_bytes_total;
        Ok(())
    }
    ...
    fn emit_stream_end(
        ...
        self.emit_event(ChatEvent::StreamEnd(StreamEndData {
            message: ChatMessage {
                ...
                tool_calls,
                model_info,
                token_usage: token_usage_from_value(usage.as_ref()),
                context_breakdown: context_breakdown_from_value(context_breakdown.as_ref()),
                images: Some(Vec::new()),
            },
        }));
    }
    ...
    fn emit_error(&self, message: &str) {
        self.emit_event(ChatEvent::Error(message.to_string()));
    }
...
fn emit_subagent_task_prompt_if_needed(stream: &mut SubAgentStream, description: &str) {
    ...
    stream
        .inner
        .emit_event(ChatEvent::MessageAdded(ChatMessage {
            timestamp: unix_now_ms(),
            sender: MessageSender::User,
...
fn tool_use_data_from_claude_tool_call(tool_call: &ClaudeToolCall) -> ToolUseData {
    ToolUseData {
...
fn token_usage_from_value(raw: Option<&Value>) -> Option<TokenUsage> {
    let usage = raw?;
    Some(TokenUsage {
        input_tokens: usage_value_u64(usage, "input_tokens"),
        output_tokens: usage_value_u64(usage, "output_tokens"),
        total_tokens: usage_value_u64(usage, "total_tokens"),
        cached_prompt_tokens: Some(usage_value_u64(usage, "cached_prompt_tokens")),
        cache_creation_input_tokens: Some(usage_value_u64(usage, "cache_creation_input_tokens")),
        reasoning_tokens: Some(usage_value_u64(usage, "reasoning_tokens")),
    })
}

fn context_breakdown_from_value(raw: Option<&Value>) -> Option<ContextBreakdown> {
    let value = raw?;
...
fn claude_task_update_from_todo_write(arguments: &Value) -> Option<ChatEvent> {
    ...
    Some(ChatEvent::TaskUpdate(TaskList {
        title: String::new(),
        tasks,
    }))
}
...
impl ClaudeRunCommandResult {
    fn as_tool_result(&self) -> ToolExecutionResult {
...
fn claude_tool_request_type(tool_name: &str, arguments: &Value) -> ToolRequestType {
    if let Some(preview) = claude_modify_preview(tool_name, arguments) {
        return ToolRequestType::ModifyFile {
    ...
    if claude_is_run_command_tool_name(tool_name) {
        ...
        let working_directory = arguments
            ...
            .unwrap_or("")
            .to_string();
        return ToolRequestType::RunCommand {
            command,
            working_directory,
        };
    }
    ...
    if claude_is_read_tool_name(tool_name) {
        return ToolRequestType::ReadFiles {
...
async fn list_claude_sessions(workspace_root: &str) -> Result<Vec<SessionMetadata>, String> {
...
fn inspect_claude_session_contents(
    ...
    Some(SessionMetadata {
        ...
        title,
        created_at: Some(resolved_created_at),
        ...
        backend_kind: Some("claude".to_string()),
    })
}
...
fn parse_claude_session_replay(contents: &str) -> ClaudeSessionReplay {
    ...
    let mut context_bytes_total = 0u64;
    ...
    for line in contents.lines() {
        ...
        let reasoning = reasoning_text.clone().map(|text| ReasoningData {
            text,
            tokens: None,
            signature: None,
            blob: None,
        ...
        let message_tool_calls: Vec<ToolUseData> = tool_calls
            .iter()
            .map(tool_use_data_from_claude_tool_call)
            .collect();
        ...
        if should_emit_message {
            context_bytes_total =
                context_bytes_total.saturating_add(estimate_message_history_bytes(&text, &images));
            ...
            restored.push(ClaudeHistoryReplayItem::Message(ChatMessage {
                timestamp: unix_now_ms(),
                sender,
                content: text,
                reasoning,
                ...
                model_info,
                token_usage: token_usage_from_value(token_usage.as_ref()),
                context_breakdown: None,
                images: Some(images),
            }));
        }
    ...
    for block in content {
        ...
        if is_error {
            ...
            if claude_is_user_input_tool_name(&tool_name) {
                let tool_result = if normalize_tool_name(&tool_name) == "exitplanmode" {
                    ...
                    match plan_content {
                        Some(content) => ToolExecutionResult::Other {
                            result: json!({ "plan_content": content }),
                        },
        ...
        if is_read_tool {
            ...
            events.push(ClaudeReplayToolExecution {
                ...
                tool_name,
                success: true,
                tool_result: ToolExecutionResult::ReadFiles { files },
                error: None,
            });
            continue;
        }
...
fn extract_images_from_content(content: &Value) -> Vec<ImageData> {
    ...
    for block in blocks {
        ...
        images.push(ImageData { media_type, data });
    }
    images
}

fn estimate_message_history_bytes(text: &str, images: &[ImageData]) -> u64 {
...
fn claude_known_models() -> Vec<ModelsListEntry> {
    let models = [
    ...
    models
        .iter()
        .map(|(id, display_name, is_default)| ModelsListEntry {
            id: (*id).to_string(),
...
fn estimate_context_breakdown(
    ...
    let observed_bytes = history_bytes
...
async fn load_claude_session_history_for_transport(
    ...
    let _ = transport;
    load_claude_session_history(workspace_root, session_id).await
}
...
mod tests {
    ...
    fn make_test_inner() -> (ClaudeInner, mpsc::UnboundedReceiver<ChatEvent>) {
        let (event_tx, event_rx) = mpsc::unbounded_channel();
...
{"type":"assistant","timestamp":"2026-04-02T19:20:23.100Z","message":{"content":[{"type":"text","text":"second"}]}}"#;
    ...
    fn make_live_test_inner(
        workspace_root: String,
    ) -> (Arc<ClaudeInner>, mpsc::UnboundedReceiver<ChatEvent>) {
        let (event_tx, event_rx) = mpsc::unbounded_channel();
        (
            Arc::new(ClaudeInner {
                event_tx,

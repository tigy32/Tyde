...
use tyde_protocol::protocol::{
    ...
    ModelsListEntry, ModuleSchemasData, OperationCancelledData, ProfilesListData, ReasoningData,
...
impl CommandExecutor for GeminiCommandHandle {
    fn execute(
        &self,
        command: SessionCommand,
    ) -> Pin<Box<dyn Future<Output = Result<(), String>> + Send + '_>> {
        Box::pin(GeminiCommandHandle::execute(self, command))
    }
}
...
impl GeminiInner {
    async fn execute_arc(this: Arc<Self>, command: SessionCommand) -> Result<(), String> {
        match command {
            ...
            SessionCommand::UpdateSettings {
                ...
                if let Some(permission_mode_value) = settings.permission_mode {
                    state.permission_mode = permission_mode_value.and_then(|permission_mode| {
                        let normalized = permission_mode.trim();
                        if normalized.is_empty() {
                            None
                        } else {
                            Some(normalized.to_string())
    ...
    async fn list_sessions(&self) -> Result<(), String> {
        ...
        let sessions: Vec<SessionMetadata> = entries
            .iter()
            .map(|entry| SessionMetadata {
                ...
                created_at: Some(entry.last_modified_ms),
                message_count: None,
                last_message_preview: Some(entry.preview.clone()),
                preview: Some(entry.preview.clone()),
                workspace_root: Some(workspace_root.clone()),
                backend_kind: Some("gemini".to_string()),
            })
            .collect();
    ...
    fn emit_summary_and_tool_requests(&self, summary: &mut GeminiStdoutSummary) -> bool {
        ...
        let tool_calls: Vec<ToolUseData> = summary
            .tool_calls
            .iter()
            .map(|tc| ToolUseData {
                id: tc.id.clone(),
        ...
        let has_output = !text.is_empty() || reasoning.is_some() || !tool_calls.is_empty();
        if !has_output {
            return false;
        }
    ...
    fn emit_event(&self, event: ChatEvent) {
        if let Err(error) = self.event_tx.send(event) {
            tracing::error!("Gemini event channel closed: {error}");
        }
    }
    ...
    fn emit_stream_start(&self, message_id: &str, model: Option<String>) {
        self.emit_event(ChatEvent::StreamStart(StreamStartData {
            ...
            agent: GEMINI_AGENT_NAME.to_string(),
            model,
        }));
    }
    ...
    fn emit_stream_end(
        ...
        let model_info = model
            ...
            .map(|value| ModelInfo { model: value });
        let token_usage = usage.as_ref().map(|value| value.token_usage.clone());
        let context_breakdown =
            estimate_gemini_context_breakdown(usage.as_ref(), model_hint.as_deref());
        ...
        "tool_result" => {
            ...
            let tool_result = if is_error {
                ToolExecutionResult::Error {
                    short_message: result_content.clone(),
                    detailed_message: result_content.clone(),
                }
            } else {
...
fn close_current_phase(
    ...
    if !text.is_empty() || reasoning.is_some() || !tool_calls.is_empty() {
...
fn gemini_tool_request_type(tool_name: &str, arguments: &Value) -> ToolRequestType {
    match tool_name {
        "replace" | "write_file" => ToolRequestType::ModifyFile {
            file_path: gemini_argument_file_path(arguments),
            before: String::new(),
            after: String::new(),
        },
        "run_shell_command" => ToolRequestType::RunCommand {
            command: arguments
                .get("command")
                .and_then(Value::as_str)
                .unwrap_or("")
        ...
        "read_file" | "read_many_files" | "list_directory" | "glob" | "grep_search" => {
            ToolRequestType::ReadFiles {
                file_paths: gemini_argument_file_paths(arguments),
            }
        }
        _ => ToolRequestType::Other {
...
fn map_tool_completion_result(tool_name: &str, result_content: &str) -> ToolExecutionResult {
    match tool_name {
        "replace" | "write_file" => ToolExecutionResult::ModifyFile {
            lines_added: 0,
            lines_removed: 0,
        },
        "run_shell_command" => ToolExecutionResult::RunCommand {
            exit_code: 0,
            stdout: result_content.to_string(),
            stderr: String::new(),
        },
        "read_file" | "read_many_files" | "list_directory" | "glob" | "grep_search" => {
            ToolExecutionResult::ReadFiles { files: Vec::new() }
        }
        _ => ToolExecutionResult::Other {
            result: Value::String(result_content.to_string()),
        },
    }
}
...
fn gemini_known_models() -> Vec<ModelsListEntry> {
    ...
    models
        .iter()
        .map(|(id, display, default)| ModelsListEntry {
            id: (*id).to_string(),
...
fn parse_gemini_usage(raw: Option<&Value>) -> Option<GeminiUsage> {
...
fn estimate_gemini_context_breakdown(
    token_usage: Option<&GeminiUsage>,
    model_hint: Option<&str>,
) -> Option<ContextBreakdown> {
    let usage = token_usage?;
    let base_input_tokens = usage.token_usage.input_tokens;
    let cached_prompt_tokens = usage.token_usage.cached_prompt_tokens.unwrap_or(0);
    let cache_creation_input_tokens = usage.token_usage.cache_creation_input_tokens.unwrap_or(0);
    let reasoning_tokens = usage.token_usage.reasoning_tokens.unwrap_or(0);
    ...
    let context_window = usage.context_window.unwrap_or_else(|| {
        std::cmp::max(
...
async fn run_gemini_cli_command_output(
    ...
    command.output().await.map_err(|err| {
        format!(
            "Failed to run Gemini CLI in '{}': {err}",
            workspace_root.trim()
        )
    })
}
...
mod tests {
    ...
    fn make_live_test_inner(
        workspace_root: String,
        ephemeral: bool,
    ) -> (Arc<GeminiInner>, mpsc::UnboundedReceiver<ChatEvent>) {
        let (event_tx, event_rx) = mpsc::unbounded_channel();
        (
            Arc::new(GeminiInner {
                event_tx,

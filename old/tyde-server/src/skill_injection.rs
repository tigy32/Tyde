...
use crate::backends::types::BackendKind;
...
pub fn list_available_skills() -> Result<Vec<String>, String> {
...
pub async fn inject_skills_for_backend(
    kind: BackendKind,
...
fn inject_skills_local(
    ...
    for name in skill_names {
        ...
        // Recreate symlink if it already exists (shared across sessions).
...
/// storage and are shared across sessions with the same agent.
pub async fn cleanup_injected_skills(cleanup: SkillCleanup) {
    if cleanup.workspace_paths.is_empty() {
        return;
    }
    let _ = cleanup.transport;
...
mod tests {
    use super::*;
    use crate::test_support::TempHomeGuard;
    use std::fs;

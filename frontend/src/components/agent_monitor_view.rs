...
pub(crate) fn status_to_filter(status: DerivedAgentState) -> AgentStatusFilter {
    match status {
        ...
        // Queued and active compaction share the `Compacting` filter bucket:
        // the filter vocabulary is persisted in user preferences, and both
        // answer the same question ("which agents are being compacted?").
        // The visible per-card label still distinguishes them.
...
mod wasm_tests {
    ...
    /// Splits are created by dragging tabs: agent monitor rows carry no
    /// open-to-the-side (duplicate) control.
    #[wasm_bindgen_test]
    async fn monitor_rows_have_no_side_open_control() {
        let agents = vec![agent("alpha", 100, false)];
        let state = primed_state(agents, AgentsViewPreferences::default());
        let mount_state = state.clone();
        let container = make_container();
        let _h = mount_to(container.clone(), move || {
        ...
        assert!(
            container
                ...
                .unwrap()
                .is_some(),
            "precondition: a monitor row renders"
        );
        assert!(
            container
                ...
                .unwrap()
                .is_none(),
            "no open-to-the-side control renders on a monitor row"
        );
        assert!(
            container
                .query_selector(".agent-monitor-open-side")
                .unwrap()
                .is_none(),

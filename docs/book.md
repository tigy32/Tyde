...
<p>This guide assumes Tyde is installed and open. It covers desktop Tyde and <a href="#mobile">access from your phone</a>. Choose a chapter for the task you are doing, or start with <a href="#first-agent">a conversation without a project</a>.</p>
<h2 id="index-interface">Find your way around the window</h2>
<figure><a href="assets/studio.png"><img src="assets/studio.png" width="3360" height="2100" alt="Tyde desktop with Garden, Atlas API, and Field Notes projects, an expanded file tree, eight agents, and a Codex conversation working on a search improvement."></a><figcaption>Browse project files, organize your agents, and follow a conversation in the same workspace. Select the image to enlarge it.</figcaption></figure>
...
<ul><li><a href="#first-agent">Start a conversation</a> and learn the response, tool, and follow-up controls.</li><li><a href="#projects">Open code in a project</a>, then <a href="#review">review its changes</a>.</li><li><a href="#concurrency">Find and organize agents</a>, or <a href="#sessions">resume an older session</a>.</li><li><a href="#remote">Connect another machine</a> and work with its projects, or <a href="#mobile">pair your phone</a> to access the same agents.</li><li><a href="#agents-skills">Configure custom agents and skills</a>, then <a href="#delegation">use subagents</a>.</li><li>Use <a href="#settings">Settings reference</a> or <a href="#troubleshooting">Troubleshooting</a> for a specific control or problem.</li></ul>
...
<p>Some backends accept their own slash commands, such as Claude Code’s <code>/compact</code> or <code>/review</code>. Type <code>/</code> in the composer to see the commands the running agent accepts, with their descriptions. Use the arrow keys or the pointer to choose one, then press <kbd>Tab</kbd> to complete it or <kbd>Enter</kbd> to send a fully typed command. Add arguments after the command name when it takes input. A message that begins with a slash but does not name an available command is sent as ordinary text.</p>
...
<h2 id="review-review-agents">Configure review aspects</h2>
<p>Open <strong>Settings → Review</strong> on the host where your agents run. Use the master switch to enable or disable reviews, and <strong>Add aspect</strong> for each focus area. Each aspect has a name, description, and instructions, independent of backend or model. Use its switch to disable it, <strong>Edit</strong> to open its configuration dialog, or <strong>Delete</strong> to remove it. Configure independent reviewer lists for Lite and Heavy. Each mode starts with one reviewer that inherits the host’s default backend and settings at launch. Add reviewers, including multiple entries using the same backend, and configure each entry’s backend, model, and reasoning settings. You can also ask the Help agent to add, edit, or remove aspects.</p>
<p>The Git sidebar shows only review counts, status, and <strong>Open review</strong>. Open it to read findings in the main workspace, grouped by file with source navigation. Expand <strong>Review actions</strong> to run a review, choose a feedback recipient, submit, or clear comments. Expand <strong>Review rounds</strong> for reviewer history and agent responses.</p>
<p><strong>Run review</strong> in the main review actions uses your selected depth against one frozen snapshot. <strong>Lite</strong> launches one agent per configured reviewer, each covering all enabled aspects. <strong>Heavy</strong> launches one agent per configured reviewer per enabled aspect: three aspects and two reviewers launch six agents, with correspondingly higher cost. Lite is the initial default; choose the default mode in Settings. Desktop, mobile, and agent requests can override it. Progress, failures, findings, and agent responses appear by aspect, reviewer, and round. Duplicate findings stay attributed rather than being automatically merged. Missing backends count as failures, not complete reviews. Historical rounds retain reviewer identities, targets, setting overrides, and the aspect instructions used at launch. <strong>Stop review</strong> interrupts the active reviewers. A failed or cancelled reviewer is incomplete, not a clean review.</p>
<p>Coding agents can call <code>tyde_request_review</code> with optional <code>mode: "light"</code> or <code>mode: "deep"</code>. The reviewers appear as children of the requesting agent, even when they use different backends. The agent calls <code>tyde_await_review</code> with the returned <code>review_id</code> and <code>round_id</code>, then reads findings with <code>tyde_get_review</code>. Completion never injects a conversation message or accepts suggestions for you. Closing the requester closes its reviewers; manually launched reviewers remain independent. The agent can fix actionable findings, record reasons with <code>tyde_review_disposition</code>, and request another round on the updated changes. <code>tyde_get_review</code> retrieves progress and history. Manual reviews retain the normal accept-and-submit flow. Enabling reviews does not trigger a paid review after every chat response.</p>
<p>Older aspect definitions retain their IDs, text, and enabled switches. Untouched execution defaults become one inherit-default reviewer per mode, even if Heavy was the selected default. Customized Light settings remain explicit; customized Heavy settings retain the old pair as ordinary editable entries. Explicitly selecting the old defaults cannot be distinguished from leaving them untouched and becomes inheritance. The selected default mode and old review history are preserved. The tool values remain <code>light</code> and <code>deep</code>, displayed as Lite and Heavy; rounds without mode metadata are labeled Legacy.</p>
...
<p>For mobile access, Tyde also has host pairing controls in settings. See <a href="#mobile">mobile access</a> for the distinction between connecting a desktop over SSH and pairing a mobile client.</p>


<h2 id="mobile">Use Tyde on your phone</h2>

<h2 id="mobile-same-agents">What the mobile app connects to</h2>
<p>Tyde mobile is a web app that connects to the Tyde host running on your computer or server. Its Agents list shows agents on that host, including conversations you opened on desktop. Open an existing agent to continue that conversation; you do not need to start another agent on the phone.</p>
<p>The host runs the CLI harnesses and holds the project files and sessions. Your phone displays the conversation and sends your actions to the host. Keep the host running, awake, and reachable. Closing the phone app does not shut down agents on the host.</p>
<h2 id="mobile-pair">Pair through the cloud</h2>
<ol><li>In desktop Tyde, open <strong>Settings → Mobile</strong>. Check the host selector: choose the computer or server whose agents you want on your phone.</li><li>Turn on <strong>Enable mobile connections</strong>, then choose <strong>Pair via cloud</strong>. Tyde displays a pairing QR and its expiry.</li><li>On your phone, open <a href="https://tycode.dev/tyde/">Tyde mobile</a> and choose the QR scanning control. Allow camera access when prompted and scan the QR on your desktop.</li><li>If scanning is unavailable, expand <strong>Show pairing URI</strong> on desktop and paste it into the phone’s pairing-code field. Choose <strong>Connect</strong>. If Tyde displays <strong>Show pairing link</strong> instead, open that link on your phone.</li><li>Complete the phone’s Tyggs sign-in and Pass flow. Managed cloud pairing requires a Tyggs Pass.</li><li>Complete the pairing prompts, then wait for the host to show as connected on the phone. Open <strong>Agents</strong> to find your conversations.</li></ol>
<p>A pairing offer expires after a short time. If it expires, create a new one on desktop. Pairing links and codes grant access to the selected host; keep them private.</p>
<h2 id="mobile-install">Add the web app to your Home Screen</h2>
<p>You can open Tyde mobile in a browser or add it to your phone’s Home Screen using the browser’s installation or sharing controls. Open the installed app and complete pairing there if it asks you to connect again. On iPhone, use the Home Screen app to enable notifications.</p>
<h2 id="mobile-conversation">Continue a desktop conversation</h2>
<ol><li>Select the paired host that owns the conversation.</li><li>Open <strong>Agents</strong> and tap the agent’s name. Wait for its conversation to load.</li><li>Read the transcript. Expand tool cards when you need to inspect what the agent ran and its result. The header shows the agent’s current status.</li><li>Type in the composer and choose <strong>Send</strong> for a follow-up. When a turn is already running, check the send-action controls: a message can be queued rather than sent as a new idle turn.</li><li>Use <strong>Stop</strong> to interrupt a running turn, or answer a question through the controls shown in the conversation.</li><li>Tap <strong>Agents</strong> in the header to return to the list. Switching conversations does not stop other agents.</li></ol>
<figure class="mobile-capture"><a href="assets/mobile-chat.png"><img src="assets/mobile-chat.png" width="1179" height="2040" alt="Tyde mobile conversation with agent Mira, an existing response, and a follow-up draft asking about keyboard navigation" loading="lazy"></a><figcaption>The agent and harness appear at the top, with messages below and a follow-up draft in the composer. Tap the image to enlarge it.</figcaption></figure>
<p>The composer also supports photo attachments when the selected harness supports image input. Use the attachment control, inspect the attached images, and send them with your message. Voice controls depend on voice being configured on the host; ordinary text chat does not require voice setup.</p>
<h2 id="mobile-navigation">Find chats and saved sessions</h2>
<table><thead><tr><th>Tab</th><th>What you do there</th></tr></thead><tbody><tr><td>Home</td><td>Check the selected host and connection overview.</td></tr><tr><td>Agents</td><td>Open an agent, choose New chat, or switch to the Teams view.</td></tr><tr><td>Sessions</td><td>Find a saved conversation and choose Resume. Load older sessions when the conversation is not in the first page.</td></tr><tr><td>Settings</td><td>Choose the theme and tool-output display, manage notifications, and inspect the selected host’s configuration.</td></tr></tbody></table>
<p>To start work from mobile, choose <strong>New chat</strong> in Agents, select an available harness and agent configuration, and send the first request. The new agent runs on the selected host and appears on desktop too. Any workspace folders you select belong to that host’s filesystem.</p>
<p>To resume earlier work, use <strong>Sessions → Resume</strong>, read the restored conversation, and then send your next instruction. Desktop calls its saved-session view <strong>History</strong>; mobile calls it <strong>Sessions</strong>.</p>
<h2 id="mobile-notifications">Enable agent notifications</h2>
<ol><li>Open <strong>Settings → Notifications</strong> on the phone. On iPhone, launch Tyde from its Home Screen icon first.</li><li>Choose <strong>Enable notifications</strong> and allow the browser’s permission request.</li><li>Check that <strong>Agent idle alerts</strong> shows <strong>On</strong>. If permission is already granted but the device is not subscribed, choose <strong>Turn on</strong>.</li><li>Use <strong>Turn off</strong> in the same section when you no longer want alerts.</li></ol>
<p>Alerts notify you when an agent finishes a turn or asks you something. If the browser reports notifications as blocked, change that site’s notification permission in browser settings. If it reports them as unavailable, follow the displayed instructions for your browser.</p>
<h2 id="mobile-reconnect">Reconnect and recover a message</h2>
<p>When you return to the phone app, check the host’s connection state before sending more work. A disconnected phone does not prove that an agent stopped: reconnect to the original host and inspect the existing conversation.</p>
<p>If Tyde displays a message-recovery control after a connection failure, read its status before resending. A message whose delivery is uncertain may already have reached the host. Check the conversation first; use the offered recovery action deliberately rather than repeatedly tapping Send.</p>
<p>If your agents or sessions seem missing, check the selected host. Pair each additional computer or server separately. Files and conversations remain with the host where they were created.</p>
<h2 id="mobile-direct">Connect through your own network</h2>
<p>Direct hosting serves the mobile web app from your own host instead of using managed cloud pairing. The phone must be able to reach the host’s address, for example on your network or VPN.</p>
<ol><li>On desktop, open <strong>Settings → Mobile</strong> for the target host and enable mobile connections.</li><li>Turn on <strong>Host the mobile app from this machine</strong>.</li><li>Configure the listening address and an HTTPS proxy for the host, then set <strong>Address phones use</strong> to the HTTPS address the phone can reach. The host serves HTTP behind the proxy; the phone needs the HTTPS address for browser features.</li><li>Wait for the direct-hosting status to report that it is serving. Released hosts include the mobile web bundle; use the bundle setting only if you are supplying a separately built bundle.</li><li>Choose <strong>Pair via local server</strong>, open its pairing link or scan the QR on the phone, and complete the direct-pairing flow.</li></ol>
<p>The cloud flow’s Tyggs Pass sign-in is separate from direct pairing. Use the local-server pairing control for direct access, and make sure the phone can reach that address when away from your desk.</p>
<h2 id="mobile-remove">Remove a phone’s access</h2>
<p>In desktop <strong>Settings → Mobile</strong>, find the phone under <strong>Paired devices</strong>, choose <strong>Remove</strong>, and confirm. That phone needs a new pairing before connecting again. To stop all mobile access to the selected host, turn off <strong>Enable mobile connections</strong>.</p>
...
<h2 id="settings-updates">Updates and agent restoration</h2>
<p>In <strong>Settings → Updates</strong>, update channel and automatic update checks apply to this device. Under <strong>Agent restoration</strong>, choose the host and toggle <strong>Resume previous agents</strong>. This is enabled by default. Turn it off to prevent that host from automatically reopening its previous agents after an update or restart. Running agents are not stopped, and saved conversations remain in History for manual resume. The preference takes effect on the next host restart.</p>
...
<h2 id="settings-codex">Codex defaults and behavior</h2>
<p>Open <strong>Settings → Codex</strong> to edit Codex’s user configuration on the selected host. These settings also apply when you use Codex outside Tyde. Project and explicit session settings can override the defaults.</p>
<ul><li><strong>Defaults:</strong> model, reasoning effort, and supported speed tiers for new chats.</li><li><strong>Subagents:</strong> enable Codex’s native subagents, set their maximum concurrency, and choose their default model and reasoning effort. The limit excludes the main agent. Tyde’s own subagent controls remain separate.</li><li><strong>Responses:</strong> communication style, verbosity, and reasoning summaries.</li><li><strong>Memory:</strong> enable memory, generate or use memories, and exclude external-context conversations from memory generation.</li><li><strong>Advanced:</strong> web search, automatic compaction threshold, tool-output token limit, and login shells.</li></ul>
<p>Changes leave running sessions alone. New chats inherit the defaults; process-level behavior takes effect the next time Codex starts, including when reopening a session. <strong>Use CLI default</strong> removes the user override. Unrelated Codex configuration is preserved, and conflicting external edits require reviewing refreshed values before saving again.</p>
...
<p><strong>Settings → Code Intelligence → Enable code intelligence</strong> controls all projects on the selected host. Turning it off stops language servers and their child processes without closing files or stopping agents. The setting persists across restarts. Rust analysis runs Cargo checks and build scripts, which can compete with your builds.</p>
...
<p>Open <strong>Settings → Mobile</strong> on desktop to enable mobile connections, pair a phone, manage paired devices, or configure direct hosting. Mobile access connects the phone to the same agents running on the selected host.</p>
<p>See <a href="#mobile">Use Tyde on your phone</a> for cloud and direct pairing, conversations, sessions, notifications, and recovery steps. Mobile pairing is separate from adding a desktop SSH host.</p>
<h2 id="settings-review">Review agents</h2>
<p>Use <strong>Review</strong> to enable reviews and define review aspects with their own instructions, choose Lite or Heavy as the default mode, and configure independent reviewer lists with their own backend/model/settings choices. Both lists initially contain one reviewer inheriting the host default. Edit aspects in a dialog or ask the Help agent to manage them. Agents await requested reviews and read findings through tools; manual reviews keep the existing submit flow. See <a href="#review-review-agents">review aspects</a>.</p>

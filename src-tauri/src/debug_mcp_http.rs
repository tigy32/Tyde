...
fn validate_png_base64(data: &str) -> Result<(), String> {
...
fn extract_valid_png_data(value: &serde_json::Value) -> Result<&str, String> {
    let data = value
        .get("data")
...
impl TydeDebugMcpServer {
    ...
    async fn tyde_debug_capture_screenshot(
        ...
        // Screenshots need a longer default timeout than other UI actions because
        // modern-screenshot must deep-clone, serialize to SVG, and render the DOM.
...
impl ServerHandler for TydeDebugMcpServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            instructions: Some(
                "Tools for debugging Tyde itself: inspect UI, capture screenshots, inspect event logs, and drive UI actions."
                    .into(),
            ),
...
pub(crate) fn stop_debug_mcp_http_server() -> bool {
    let running = RUNNING_DEBUG_MCP_HTTP_SERVER.lock().take();
...
pub(crate) fn is_debug_mcp_http_server_running() -> bool {
    RUNNING_DEBUG_MCP_HTTP_SERVER.lock().is_some()
}

pub(crate) fn debug_mcp_http_server_url() -> Option<String> {
    RUNNING_DEBUG_MCP_HTTP_SERVER
        .lock()
        .as_ref()

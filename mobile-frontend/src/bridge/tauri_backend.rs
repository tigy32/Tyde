//! Tauri (native iOS shell) bridge backend.
//!
//! Reaches the native `mobile-shell` Rust commands/events through
//! `window.__TAURI__`. This is the original bridge implementation, moved behind
//! the runtime-detected façade in [`super`]; it is selected whenever the wasm
//! bundle runs inside the Tauri WKWebView. Behaviour is unchanged.
...
async fn listen_event(
    ...
    let unlisten = JsFuture::from(promise)
        .await
        .map_err(|e| format!("{e:?}"))?;
    Ok(UnlistenHandle::from_cleanup(move || {
        // Dropping the closure unregisters the Rust side; the JS unlisten fn
        // removes the listener on the Tauri side.
        drop(closure);
        if let Ok(callback) = unlisten.dyn_into::<js_sys::Function>() {

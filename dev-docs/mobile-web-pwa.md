...
connection's local queue. Callers never wait for a broker acknowledgement, and
...
- `BrokerAcknowledged`: the broker acknowledged the serialized write and flush
  for that line.
- `DeliveryUnknown`: the writer dequeued the line, then the session ended,
  failed, or timed out before `BrokerAcknowledged`.

**A PUBACK is never called delivered. `BrokerAcknowledged` makes no claim that
...
to one `BrokerAcknowledged` submission without changing `mqtt-transport`.
...
existing `MqttReconnectBackoff`. Work from the dead connection is dropped and
...
in-flight records remain silent; `BrokerAcknowledged` retires them silently.
...
- `mobile-frontend/src/bridge.rs` — the web-only bridge;
  transport/storage/QR go direct-to-WASM.
- Git history for the removed native shell — reference behavior used while
  porting the connection manager, PSK store, and host store to WASM.
...
3. Switch `DEFAULT_MOBILE_MQTT_BROKER_URL` to the shared `wss` endpoint and
   update host QR generation (`server/src/mobile_access.rs`) to publish a
   wss-reachable broker. Update the

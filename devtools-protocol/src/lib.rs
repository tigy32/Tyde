...
mod tests {
    ...
    fn disposable_hermes_environment_is_confined_and_seeds_loopback_stub() {
        ...
        let resolved_store = store.path().canonicalize().expect("resolve store");
        assert!(prepared.home.starts_with(&resolved_store));
        ...
        assert_eq!(
            prepared.attestation.home_ephemeral,
            prepared.home.starts_with(&resolved_store)
        );
        assert_eq!(
            prepared.attestation.hermes_home_ephemeral,
            prepared.hermes_home.starts_with(&prepared.home)
        );
        assert_eq!(
            prepared.attestation.loopback_stub_url,
    ...
    fn disposable_hermes_environment_rejects_egress_and_profile_traversal() {
        ...
        let egress = prepare_disposable_hermes_environment(
            ...
            &DisposableHermesEnvironment {
                profiles: Vec::new(),
                loopback_stub: HermesLoopbackStub {
                    base_url: "http://8.8.8.8:8080/v1".to_owned(),
        ...
        assert!(egress.contains("must use 127.0.0.1"));

        let traversal_store = tempfile::tempdir().expect("traversal store");
    ...
    #[test]
    fn disposable_hermes_environment_requires_loopback_stub() {
        let error = serde_json::from_value::<DisposableHermesEnvironment>(serde_json::json!({
            "profiles": ["qa"]
        }))
        .expect_err("contained Hermes mode without a stub must be rejected");
        assert!(error.to_string().contains("loopbackStub"));
        let schema = disposable_hermes_environment_json_schema();
        assert!(
            schema["required"]
                .as_array()
                .expect("required fields")
                .contains(&Value::String("loopbackStub".to_owned()))
        );
    }

    #[test]
    fn parent_runtime_resolves_home_dependent_hermes_launcher() {
        ...
        let agent = test_executable(agent_bin.join("hermes"));
        test_executable_with_contents(
            private_bin.join("hermes-private-env"),
            "#!/bin/sh\nHOME_ROOT=\"$HOME/.hermes\"\nexec \"$@\"\n",
        );
        test_executable_with_contents(
            hermes_home.join("hermes-private"),
            "#!/bin/sh\nexec \"$HOME/.hermes/bin/hermes-private-env\" \"$HOME/.hermes/hermes-agent/venv/bin/hermes\" \"$@\"\n",
        );
        let launcher = test_executable_with_contents(
            bin.join("hermes"),
            "#!/bin/sh\nexec \"$HOME/.hermes/hermes-private\" \"$@\"\n",
        );
        ...
        .expect("resolve parent Hermes runtime");
    ...
    #[test]
    fn provider_environment_classifier_covers_credentials_and_endpoint_overrides() {
        for key in [
            "AWS_ACCESS_KEY_ID",
            "AWS_SECRET_ACCESS_KEY",
            "AWS_SESSION_TOKEN",
            ...
            "FUTURE_PROVIDER_BASE_URL",
            "FUTURE_PROVIDER_API_HOST",
        ] {
            assert!(
                is_provider_environment_key(OsStr::new(key)),
        ...
        for key in [
            ...
            assert!(
                !is_provider_environment_key(OsStr::new(key)),
                "{key} must remain available to the launcher"
            );
        }
    }

    #[test]
    fn startup_cleanup_removes_every_tracked_artifact_unless_disarmed() {
        let root = tempfile::tempdir().expect("cleanup root");
        let leaked_store = root.path().join("leaked-store");
        let leaked_config = root.path().join("leaked-config.json");
        fs::create_dir(&leaked_store).expect("create leaked store");
        fs::write(&leaked_config, "{}").expect("write leaked config");
        {
            let mut cleanup = DevInstanceStartupCleanup::new(leaked_store.clone());
            cleanup.track_config(leaked_config.clone());
        }
        assert!(!leaked_store.exists());
        assert!(!leaked_config.exists());

        let live_store = root.path().join("live-store");
        fs::create_dir(&live_store).expect("create live store");
        {
            let mut cleanup = DevInstanceStartupCleanup::new(live_store.clone());
            cleanup.disarm();
        }
    ...
    fn test_executable(path: PathBuf) -> PathBuf {
        test_executable_with_contents(path, "#!/bin/sh\nexit 0\n")
    }

    fn test_executable_with_contents(path: PathBuf, contents: &str) -> PathBuf {
        fs::write(&path, contents).expect("write executable");
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            let mut permissions = path.metadata().expect("executable metadata").permissions();

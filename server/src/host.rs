...
use crate::agent::{
    ...
    RelayAgentRuntimeResources, RelayEventReceivers, derive_agent_name,
...
/// An agent asking to be compacted. The actor decides *whether* to compact —
/// it is the only place that knows its own context size and idleness — but
/// compaction can rotate the agent into a replacement, and only the host owns
/// the registry that swap happens in.
...
pub(crate) type SupervisorCompactionTx = mpsc::UnboundedSender<SupervisorCompactionRequest>;
type HostCapacityRx = mpsc::UnboundedReceiver<HostCapacityUpdate>;
#[derive(Clone)]
pub(crate) struct HostSessionSummaryCountUpdate {
    pub agent_id: AgentId,
...
/// How long the host waits for an auto-compaction an agent requested to
...
impl HostHandle {
    ...
    async fn close_agent_with_host_visibility(
        ...
        if let Err(error) = session_store
            .lock()
            .await
            .clear_restore_states(&closing_session_ids)
        {
            tracing::error!(
                error = %error,
    ...
    pub(crate) async fn open_project_path(
        ...
        let (project_store, opener_program) = {
            ...
            let opener_program: Option<PathBuf> = None;
...
fn spawn_host_inner(
    ...
    let (supervisor_compaction_tx, supervisor_compaction_rx) =
        mpsc::unbounded_channel::<SupervisorCompactionRequest>();
...
impl HostHandle {
    async fn restore_open_agents(&self) -> Result<(), String> {
        let (session_store, team_registry, antigravity_conversations_dir) = {
            let state = self.state.lock().await;
            (
                Arc::clone(&state.session_store),
                state.team_registry.clone(),
...
/// Performs the compactions agents ask for. All that is left on the host side
/// of supervision: the decision belongs to the actor, but a legacy compaction
/// rotates the agent into a replacement, and only the host owns that registry.
fn spawn_supervisor_compaction_executor(
    host: HostHandle,
    ...
    let worker = async move {
        while let Some(request) = requests.recv().await {
            ...
            let stream = Stream::new(
                StreamPath(format!("/agent/{}/supervisor-compact", request.agent_id)),
                tx,
            );
            match host
                .compact_agent_if_inactive_in_background(
                    request.agent_id.clone(),
                    request.activity_counter,
                    request.settings_epoch,
                    AgentCompactPayload {
                        summary_prompt: None,
                ...
                .await
            {
                Ok(accepted) => tracing::info!(
                    ...
                    accepted,
                    "supervisor auto-compaction requested by the agent"
                ),
                Err(error) => tracing::warn!(
                    agent_id = %request.agent_id,
                    error = %error,

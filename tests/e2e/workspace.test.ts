...
describe("Multi-workspace management", () => {
  ...
  it("comprehensive workspace flow: welcome → tab isolation → settings overlay", async () => {
    ...
    const workspaceALabels = await browser.execute(
      (viewSel, tabBarSel, tabTitleSel) => {
        const activeView = Array.from(document.querySelectorAll(viewSel)).find(
          (el) => (el as HTMLElement).style.display !== "none",
        );
        if (!activeView) return [];
        return Array.from(
          activeView.querySelectorAll(`${tabBarSel} ${tabTitleSel}`),
    ...
    const workspaceBLabels = await browser.execute(
      (viewSel, tabBarSel, tabTitleSel) => {
        const activeView = Array.from(document.querySelectorAll(viewSel)).find(
          (el) => (el as HTMLElement).style.display !== "none",
        );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "No visible welcome screen on second workspace",
      },
    );
    ...
    await browser.waitUntil(
      async () =>
        (await title.getText()).includes("workspace") &&
        !(await title.getText()).includes("workspace-b"),
      { timeout: 10_000, timeoutMsg: "Workspace A did not restore" },
    );

    // Workspace A's original tabs are restored
    const restoredLabels = await browser.execute(
      (viewSel, tabBarSel, tabTitleSel) => {
        const activeView = Array.from(document.querySelectorAll(viewSel)).find(
          (el) => (el as HTMLElement).style.display !== "none",
        );
    ...
    const settingsResult = await browser.execute(
      (settingsViewSel, viewSel) => {
        const settings = document.querySelector(settingsViewSel);
        if (!settings)
          return { exists: false, insideWorkspace: false, parentId: null };
        let el = settings.parentElement;
        let insideWorkspace = false;
        while (el) {
          if (el.matches(viewSel)) {
        ...
        return {
          exists: true,
          insideWorkspace,
          parentId: settings.parentElement?.id ?? null,
        };
      },
      sel.settingsTabView,
      sel.workspaceView,
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 10_000,
        timeoutMsg: "Workspace B did not load for settings test",
      },
    );
  ...
  it("recovers from malformed persisted project roots without a blank workspace", async () => {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 10_000,
        timeoutMsg: "Workspace did not recover from malformed persisted state",
      },
    );

    const workspaceView = await $(
      '[data-testid="workspace-view"][data-view-mode="workspace"]',
    );

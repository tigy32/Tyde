...
async function spawnFeedbackAgent(
  filePath: string,
  lineContent: string,
  feedback: string,
): Promise<number> {
  const result = await browser.executeAsync(
    (
      ...
      if (!spawn) {
        (done as any)({
          ok: false,
          error: "Missing __test_spawnFeedbackAgent hook",
        });
        return;
      }
      spawn(path, line, note)
        ...
        .catch((err: unknown) =>
          (done as any)({ ok: false, error: String(err) }),
        );
    },
    filePath,
    lineContent,
...
async function openAgentsWidget(): Promise<void> {
  ...
  await browser.execute((tabSel: string) => {
    ...
    const tab = tabs.find(
      (el) =>
        el.offsetParent !== null && (el.textContent ?? "").trim() === "Agents",
    );
    tab?.click();
  }, sel.dockWidgetTab);
}
...
async function openSessionsWidget(): Promise<void> {
  ...
  await browser.execute((tabSel: string) => {
    ...
    const tab = tabs.find(
      (el) =>
        el.offsetParent !== null && (el.textContent ?? "").trim() === "History",
    );
    tab?.click();
  }, sel.dockWidgetTab);
}
...
async function clickVisibleSessionsRefresh(timeoutMsg: string): Promise<void> {
  await browser.waitUntil(
    async () =>
      browser.execute((buttonSel: string) => {
        const buttons = Array.from(
          document.querySelectorAll(buttonSel),
...
async function setVisibleSessionsSearch(value: string): Promise<void> {
  await browser.waitUntil(
    async () =>
      browser.execute(
        (panelSel: string, searchValue: string) => {
          const panels = Array.from(
            document.querySelectorAll(panelSel),
          ) as HTMLElement[];
          const panel = panels.find((node) => {
...
async function getVisibleSessionCardTexts(): Promise<string[]> {
  return browser.execute(
    (panelSel: string, cardSel: string) => {
      const panels = Array.from(
        document.querySelectorAll(panelSel),
      ) as HTMLElement[];
      const panel = panels.find((node) => {
      ...
      const cards = Array.from(
        panel.querySelectorAll(cardSel),
      ) as HTMLElement[];
...
describe("Message queue, steer, and interrupt", () => {
  it("comprehensive queue lifecycle: queue while typing → remove → drain → steer → button states", async () => {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 3000,
        timeoutMsg: "Cancel button should be enabled when typing",
      },
    );
    expect(await cancelBtn.getText()).toBe("Interrupt");
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 3000,
        timeoutMsg: "Cancel button should show Steer when typing + text",
      },
    );
    ...
    await browser.execute(
      (el: HTMLElement) => (el as HTMLButtonElement).click(),
      sendBtn,
    );
    ...
    await browser.waitUntil(
      async () =>
        !(await queueIndicator.getAttribute("class"))?.includes("hidden"),
      {
        timeout: 3000,
        timeoutMsg: "Queue indicator should be visible after queuing",
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 3000,
        timeoutMsg: "Cancel button should be disabled when idle",
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 3000,
        timeoutMsg: "Steered item should be removed from queue",
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 3000,
        timeoutMsg: "Cancel should show Steer with text + typing",
      },
    );
...
describe("Chat scroll behavior", () => {
  it("comprehensive scroll lifecycle: auto-scroll → scroll up → button → new messages → tool completion → stream end", async () => {
    ...
    const _chatContainer = await $(sel.chatContainer);
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Chat container should overflow after 20 messages",
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Scroll-to-bottom button should appear when scrolled up",
      },
    );
    ...
    await emitChatEvent(WORKSPACE_CONV_ID, "MessageAdded", {
      sender: { Assistant: { agent: "tycode" } },
      content:
        "New message while user is scrolled up — should not move scroll position.",
      timestamp: Date.now(),
      images: [],
      token_usage: null,
      reasoning: null,
    ...
    await browser.waitUntil(async () => (await getScrollState()).atBottom, {
      timeout: 3000,
      timeoutMsg: "Should be at bottom after clicking scroll-to-bottom",
    });

    await browser.waitUntil(
      async () =>
        (await scrollBtn.getAttribute("class"))?.includes("hidden") ?? false,
      {
        timeout: 3000,
        timeoutMsg: "Scroll button should hide after clicking it",
      },
    );
    ...
    await emitChatEvent(WORKSPACE_CONV_ID, "StreamStart", {
      agent: "tycode",
      model: null,
    });
    await emitChatEvent(WORKSPACE_CONV_ID, "StreamDelta", {
      text: "Streaming content that grows the container. ",
    });
    await emitChatEvent(WORKSPACE_CONV_ID, "StreamDelta", {
      text: "More streaming content to ensure height increases. ",
    });
    await emitChatEvent(WORKSPACE_CONV_ID, "StreamDelta", {
    ...
    await emitChatEvent(WORKSPACE_CONV_ID, "StreamEnd", {
      message: makeAssistantMessage(
        "Streaming content that grows the container. More streaming content to ensure height increases. Even more content.\n\nWith paragraphs.",
      ),
    });
    ...
    await emitChatEvent(WORKSPACE_CONV_ID, "MessageAdded", {
      sender: { Assistant: { agent: "tycode" } },
      content:
        "Message added while this chat tab is inactive — should still be at bottom when reopened.",
      timestamp: Date.now(),
      images: [],
      token_usage: null,
      reasoning: null,
    ...
    await browser.execute((tabSel: string) => {
      const tabs = Array.from(
        document.querySelectorAll(tabSel),
      ) as HTMLElement[];
      tabs[0]?.click();
    }, sel.convTab);
    ...
    await browser.waitUntil(async () => (await getScrollState()).atBottom, {
      timeout: 3000,
      timeoutMsg:
        "Should be at bottom after switching back to tab with new message",
    });
    expect(await scrollBtn.getAttribute("class")).toContain("hidden");
  });
});
...
describe("Reasoning rendering", () => {
  it("preserves streamed reasoning when StreamEnd omits the final reasoning payload", async () => {
    ...
    await browser.waitUntil(
      async () =>
        browser.execute((assistantSel: string) => {
          const messages = Array.from(
            document.querySelectorAll(assistantSel),
          ) as HTMLElement[];
          if (messages.length === 0) return false;
          const last = messages[messages.length - 1];
          const text = last.textContent ?? "";
          return (
            text.includes("Checking workspace constraints before editing.") &&
        ...
        }, sel.assistantMessage),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected streamed reasoning fallback to render in final assistant message",
      },
    );
  });
});
...
describe("Chat tab lifecycle", () => {
  it("comprehensive chat flow: welcome → messages → context → tools → sessions → error handling", async () => {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Streaming bubble should finalize after StreamEnd",
      },
    );
    ...
    await emitChatEvent(99999, "StreamEnd", {
      message: makeAssistantMessage("Ghost message"),
    });
    ...
    const _errorMsg = await $(sel.errorMessage);
    await browser.waitUntil(
      async () => {
      ...
      {
        timeout: 5000,
        timeoutMsg: 'Error message should contain "Something went wrong"',
      },
    );
      ...
      message: makeAssistantMessage("I will use two tools.", [
        {
          id: toolCallId1,
          name: "ReadFiles",
          arguments: { file_paths: ["/mock/file.ts"] },
        },
        {
          id: toolCallId2,
          name: "ModifyFile",
          arguments: { file_path: "/mock/broken.ts" },
        },
      ]),
    });
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Two pending tool cards should appear after StreamEnd",
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 3000,
        timeoutMsg: "First tool should transition to Running...",
      },
    );
    ...
    await emitChatEvent(WORKSPACE_CONV_ID, "ToolExecutionCompleted", {
      ...
      tool_result: {
        kind: "ReadFiles",
        files: [{ path: "/mock/file.ts", bytes: 100 }],
      },
      success: true,
    });
    ...
    const errorVisible = await browser.execute((toolCardSel: string) => {
      ...
      return (
        lastCard?.textContent?.includes("File not found: /mock/broken.ts") ??
        false
      );
    }, sel.toolCard);
    ...
    const toolResult = await browser.execute(
      (
        _assistantSel: string,
        embeddedSel: string,
        toolCardSel: string,
        chatContainerSel: string,
      ) => {
        let orphanedToolCalls = 0;
        for (const chatContainer of document.querySelectorAll(
          chatContainerSel,
        )) {
          const orphaned = chatContainer.querySelector(
            `:scope > ${embeddedSel}`,
          );
          if (orphaned) {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Tool card did not appear for mock-driven message",
      },
    );
    ...
    await emitChatEvent(WORKSPACE_CONV_ID, "StreamDelta", {
      text: "Running a command for disconnect test.",
    });
    await emitChatEvent(WORKSPACE_CONV_ID, "StreamEnd", {
    ...
    await browser.waitUntil(
        ...
        }),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected RunCommand tool result to show stdout in compact mode",
      },
    );
    ...
    await browser.execute(() => {
      const btn = document.querySelector(
        '[data-testid="tool-output-toggle-global"]',
    ...
    await browser.waitUntil(
        ...
        }),
      {
        timeout: 3000,
        timeoutMsg:
          "Tool result should reflect summary mode after disconnect/broadcast/reconnect",
      },
    );
    ...
    await browser.waitUntil(
        ...
        }),
      {
        timeout: 3000,
        timeoutMsg:
          "Tool result should re-render stdout after switching back to compact",
      },
    );
      ...
      message: makeAssistantMessage("Running a long command.", [
        {
          id: bgToolId,
          name: "RunCommand",
          arguments: { command: "cat long.txt" },
        },
      ]),
    });
    await emitChatEvent(WORKSPACE_CONV_ID, "ToolRequest", {
    ...
    await browser.execute((tabSel: string) => {
      ...
      const firstTab = tabs.find(
        (t) => !t.classList.contains("conv-tab-active"),
      );
      firstTab?.click();
    }, sel.convTab);
    ...
    await browser.waitUntil(
      async () =>
        browser.execute(() => {
          const results = document.querySelectorAll(".tool-result-command");
          const last = results[results.length - 1] as HTMLElement;
          const truncatable = last
            ?.closest(".embedded-tool-calls")
            ?.querySelector(".truncatable");
          return truncatable?.classList.contains("collapsed") ?? false;
        }),
      {
        timeout: 5000,
        timeoutMsg:
          "Truncatable tool output should keep .collapsed after tab switch",
      },
    );
    ...
    await browser.execute((tabSel: string) => {
      ...
      const closeBtn = secondTab?.querySelector(
        ".conv-tab-close",
      ) as HTMLElement | null;
      closeBtn?.click();
    }, sel.convTab);
    ...
    const askPendingCount = await browser.execute((toolStatusSel: string) => {
      ...
      return Array.from(statuses).filter(
    ...
    await emitChatEvent(WORKSPACE_CONV_ID, "ToolRequest", {
      ...
      tool_type: {
        kind: "Other",
        args: { tool: "ExitPlanMode", arguments: {} },
      },
    });
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 3000,
        timeoutMsg: 'ExitPlanMode should render as "Plan ready" indicator',
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 3000,
        timeoutMsg: "ExitPlanMode should render plan content as markdown",
      },
    );
    ...
    const planCardInfo = await browser.execute(() => {
      ...
      const status = card
        ?.querySelector(".tool-status-text")
        ?.textContent?.trim();
      return { name, status };
    });
    ...
    const planHasStuckStatus = await browser.execute(() => {
      ...
      const statusTexts = Array.from(
        container.querySelectorAll(".tool-status-text"),
      ).map((el) => el.textContent?.trim());
      return (
        statusTexts.includes("Running...") || statusTexts.includes("Pending")
      );
    });
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected visible sessions panel in workspace view",
      },
    );
    await clickVisibleSessionsRefresh(
      "Visible sessions refresh button was not clickable",
    );
    ...
    await browser.waitUntil(
      async () => {
        ...
        return (
          text.includes("Tycode Session 1") &&
      ...
      {
        timeout: 5000,
        timeoutMsg: "Sessions panel should display merged backend session data",
      },
    );

    await clickVisibleSessionsRefresh(
      "Visible sessions refresh button was not clickable on second click",
    );
    await browser.waitUntil(
      async () => {
        const text = await getVisibleSessionsPanelText();
        return (
          text.includes("Tycode Session 1") && text.includes("Codex Session 1")
        );
      },
      {
        timeout: 5000,
        timeoutMsg: "Sessions should reload after second refresh",
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Send button should be disabled after SubprocessExit",
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Send button should be re-enabled after relaunch",
      },
    );
  ...
  it("clears stale context usage when the latest assistant message has no breakdown", async () => {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected initial context usage to be visible",
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Context usage should clear when breakdown is missing",
      },
    );
...
describe("Sessions backend behavior", () => {
  it("loads merged backend sessions, resumes into a new tab, and deletes only the selected backend session", async () => {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected merged Tycode and Codex sessions",
      },
    );

    const initialChatTabCount = await browser.execute(() => {
      return document.querySelectorAll(
        '[data-testid="conv-tab"]:not(.conv-tab-file)',
      ).length;
    });
    ...
    await browser.execute(() => {
      const cards = Array.from(
        document.querySelectorAll('[data-testid="session-card"]'),
      ) as HTMLElement[];
      const codexCard = cards.find((card) =>
        (card.textContent ?? "").includes("Codex Session 1"),
      );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Resuming a session should always open a new chat tab",
      },
    );

    await clickVisibleSessionsRefresh(
      "Visible sessions refresh button was not clickable before delete check",
    );
    await browser.waitUntil(
      async () => {
        const text = await getVisibleSessionsPanelText();
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected both backend sessions before delete",
      },
    );
    ...
    await browser.execute((tabTitleSel: string) => {
      const activeTab = document.querySelector(
        '[data-testid="conv-tab"].conv-tab-active',
      );
      const titleEl = activeTab?.querySelector(
        tabTitleSel,
      ) as HTMLElement | null;
    ...
    await browser.waitUntil(
      async () => {
        const title = await (
          await $(`${sel.convTabActive} ${sel.convTabTitle}`)
        ).getText();
    ...
    await clickVisibleSessionsRefresh(
      "Visible sessions refresh button was not clickable after rename",
    );
    await browser.waitUntil(
      async () => {
        const text = await getVisibleSessionsPanelText();
      ...
      {
        timeout: 5000,
        timeoutMsg: "Sessions panel should show the renamed alias",
      },
    );
    ...
    await browser.waitUntil(
      async () => {
        ...
        return (
          cardTexts.length > 0 &&
          cardTexts.some((text) => text.includes("My Codex Chat"))
        );
      },
    ...
    await browser.waitUntil(
      async () => {
        ...
        return (
          cardTexts.length > 0 &&
          cardTexts.every((text) => !text.includes("Tycode Session 1"))
        );
      },
      {
        timeout: 3000,
        timeoutMsg: "Search should filter out non-matching sessions",
      },
    );
    ...
    await clickVisibleSessionsRefresh(
      "Visible sessions refresh button was not clickable before delete",
    );
    await browser.waitUntil(
      async () =>
        (await getVisibleSessionsPanelText()).includes("My Codex Chat"),
    ...
    await browser.execute(() => {
      ...
      const codexCard = cards.find((card) =>
        (card.textContent ?? "").includes("My Codex Chat"),
      );
      if (!codexCard)
        throw new Error("Missing Codex session card before delete");
      const deleteBtn = codexCard.querySelector(
        'button[title="Delete session"]',
      ) as HTMLButtonElement | null;
      if (!deleteBtn)
        throw new Error("Missing delete button on Codex session card");
      deleteBtn.click();
    });
    ...
    await browser.waitUntil(
      async () => {
        ...
        return (
          !text.includes("Codex Session 1") &&
          !text.includes("My Codex Chat") &&
          text.includes("Tycode Session 1")
        );
      },
      {
        timeout: 5000,
        timeoutMsg: "Deleting Codex session should not remove Tycode sessions",
      },
    );
...
describe("Claude resume parity", () => {
  it("renders replayed Claude tool cards and image attachments from restored history", async () => {
    ...
    await browser.waitUntil(
      async () =>
        browser.execute((statusSel: string) => {
          const statuses = Array.from(document.querySelectorAll(statusSel)).map(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected resumed Claude tool card to reach Done state",
      },
    );

    const hasUserImage = await browser.execute((chatMessageSel: string) => {
      const messages = Array.from(
        document.querySelectorAll(chatMessageSel),
      ) as HTMLElement[];
    ...
    const orphanedToolCards = await browser.execute(
      (chatContainerSel: string, embeddedSel: string, toolCardSel: string) => {
        let orphaned = 0;
        for (const chatContainer of document.querySelectorAll(
          chatContainerSel,
        )) {
          const directToolContainer = chatContainer.querySelector(
            `:scope > ${embeddedSel}`,
          );
          if (directToolContainer) {
            orphaned +=
              directToolContainer.querySelectorAll(toolCardSel).length;
          }
        }
        return orphaned;
      },
  ...
  it("anchors replayed Claude diffs to the original assistant message and opens the diff viewer", async () => {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected replayed Claude diff button to appear",
      },
    );
    ...
    await browser.execute(() => {
      const firstDiffBtn = document.querySelector(
        ".assistant-message .view-diff-btn",
      ) as HTMLButtonElement | null;
      firstDiffBtn?.click();
    });
    ...
    await browser.waitUntil(
      async () =>
        browser.execute(() => {
          ...
          const title =
            active
              ?.querySelector('[data-testid="conv-tab-title"]')
              ?.textContent?.trim() ?? "";
          return title === "app.ts";
        }),
      {
        timeout: 5000,
        timeoutMsg: "Expected diff tab to open for replayed Claude edit",
      },
    );
    ...
    await browser.waitUntil(
      async () =>
        browser.execute(() => {
          const removed = Array.from(
            document.querySelectorAll(
              ".diff-panel-removed .diff-panel-line-text",
            ),
          ).map((node) => node.textContent ?? "");
          const added = Array.from(
            document.querySelectorAll(
              ".diff-panel-added .diff-panel-line-text",
            ),
          ).map((node) => node.textContent ?? "");
          return (
            removed.some((line) => line.includes("const value = 1;")) &&
        ...
        }),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected replayed Claude diff contents to render in diff viewer",
      },
    );
  });
});
...
describe("Agents panel parity", () => {
  it("shows normal chat conversations in the agents panel", async () => {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected at least one agent card after creating a chat",
      },
    );
    await browser.waitUntil(
      async () => (await (await $$(sel.agentCardRunning)).length) >= 1,
      {
        timeout: 5000,
        timeoutMsg:
          "Expected agent to remain running until TypingStatusChanged=false",
      },
    );
    await browser.waitUntil(
      async () =>
        browser.execute(
          (runningSel: string, interruptSel: string) => {
            const runningCards = Array.from(
              document.querySelectorAll(runningSel),
            ) as HTMLElement[];
          ...
          sel.agentCardRunning,
          sel.agentCardInterrupt,
        ),
      {
        timeout: 5000,
        timeoutMsg: "Expected Interrupt action on running conversation card",
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected no running cards after TypingStatusChanged=false",
      },
    );
    await browser.waitUntil(
      async () =>
        browser.execute(
          (cardSel: string, removeSel: string) => {
            const cards = Array.from(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected Remove action on completed conversation card",
      },
    );
  ...
  it("retains feedback-agent conversation history when opened after background execution", async () => {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected no running agent cards after typing has stopped",
      },
    );

    await browser.execute((cardSel: string) => {
      const card = Array.from(document.querySelectorAll(cardSel)).find(
    ...
    await browser.waitUntil(
      ...
      },
      {
        timeout: 5000,
        timeoutMsg:
          "Expected assistant history with mock response text when opening feedback agent",
      },
    );
    ...
    expect(latestText).toContain(
      "Mock response to: Apply the following feedback to file: /mock/workspace/README.md",
    );
    ...
    await browser.execute(() => {
      const fileTab = document.querySelector(
        '[data-testid="conv-tab"].conv-tab-file',
      ) as HTMLElement | null;
      fileTab?.click();
    });
    ...
    await browser.waitUntil(
      async () =>
        browser.execute(
          () => document.querySelector(".feedback-complete-icon") !== null,
        ),
      {
        timeout: 5000,
        timeoutMsg: "Expected feedback box to show completion icon",
      },
    );
      ...
      async () =>
        browser.execute(() => {
          ...
          return Array.from(lines).some((el) =>
            (el.textContent ?? "").includes("Updated heading"),
          );
        }),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected file tab content to be refreshed with updated content",
      },
    );
  });
  ...
  it("shows runtime agents in both the project and home agents views and supports interrupt/remove controls", async () => {
    ...
    await browser.waitUntil(
      ...
      },
      {
        timeout: 5000,
        timeoutMsg:
          "Expected runtime agent to appear in the project Agents widget",
      },
    );
    ...
    await browser.waitUntil(
      async () =>
        browser.execute(
          (intSel: string, termSel: string) => {
            const visible = (s: string) =>
              Array.from(document.querySelectorAll(s)).find(
                (el) => (el as HTMLElement).offsetParent !== null,
              ) as HTMLElement | undefined;
            return !!(visible(intSel) && visible(termSel));
          },
          sel.agentCardInterrupt,
          sel.agentCardTerminate,
        ),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected interrupt and terminate buttons to be visible on the runtime agent card",
      },
    );
    await browser.execute((intSel: string) => {
      const btn = Array.from(document.querySelectorAll(intSel)).find(
        (el) => (el as HTMLElement).offsetParent !== null,
      ) as HTMLElement | undefined;
      btn?.click();
    }, sel.agentCardInterrupt);
    ...
    await browser.waitUntil(
      async () => (await (await $$(sel.agentCardRemove)).length) > 0,
      {
        timeout: 5000,
        timeoutMsg:
          "Expected Remove button after interrupting the runtime agent",
      },
    );
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected runtime agent to appear in the home Agents tab",
      },
    );

    await browser.execute(
      (cardSel: string, buttonSel: string) => {
        const card = Array.from(document.querySelectorAll(cardSel)).find(
          (el) =>
            (el as HTMLElement).offsetParent !== null &&
            (el.textContent ?? "").includes("Bridge Worker"),
        );
    ...
    await browser.waitUntil(
      ...
      },
      {
        timeout: 5000,
        timeoutMsg:
          "Expected removed runtime agent to disappear from the home Agents tab",
      },
    );
    ...
    await browser.waitUntil(
      ...
      },
      {
        timeout: 5000,
        timeoutMsg:
          "Expected removed runtime agent to disappear from the project Agents widget",
      },
    );
  });
  ...
  it("marks a bridge sub-agent as completed when TypingStatusChanged false is received", async () => {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected sub-agent card to appear in agents panel",
      },
    );
    await browser.waitUntil(
      async () => (await (await $$(sel.agentCardRunning)).length) >= 1,
      {
        timeout: 5000,
        timeoutMsg: "Expected sub-agent card to show running state",
      },
    );
    ...
    await browser.waitUntil(
      async () =>
        browser.execute(
          (cardSel: string, titleSel: string) => {
            const cards = Array.from(
              document.querySelectorAll(cardSel),
            ) as HTMLElement[];
            return !cards.some((card) => {
              ...
              return (
                title?.textContent?.trim() === "Sub-agent Worker" &&
        ...
        ),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected sub-agent card to stay completed after syncRuntimeAgents poll",
      },
    );
    await browser.waitUntil(
      async () =>
        browser.execute(
          (cardSel: string, removeSel: string, titleSel: string) => {
            const cards = Array.from(
              document.querySelectorAll(cardSel),
            ) as HTMLElement[];
            return cards.some((card) => {
              if (!card.offsetParent) return false;
              const title = card.querySelector(titleSel);
              if (!title || title.textContent?.trim() !== "Sub-agent Worker")
                return false;
              return Boolean(card.querySelector(removeSel));
            });
          },
        ...
        ),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected Remove button on completed sub-agent card after poll",
      },
    );
  });
  ...
  it("reopens hidden agent chats from the agents widget without duplicating tabs or losing live updates", async () => {
    ...
    await emitChatEvent(
      WORKSPACE_CONV_ID,
      "MessageAdded",
      makeAssistantMessage("Background update while hidden"),
    );
    ...
    await browser.waitUntil(
      async () => (await (await $$(sel.agentCard)).length) >= 1,
      {
        timeout: 5000,
        timeoutMsg:
          "Expected the hidden conversation to remain in the agents panel",
      },
    );
    ...
    await browser.waitUntil(
      async () => (await (await $$(sel.convTab)).length) === 1,
      {
        timeout: 5000,
        timeoutMsg:
          "Expected clicking the agent card to unhide a single chat tab",
      },
    );
    ...
    await browser.waitUntil(
      ...
      },
      {
        timeout: 5000,
        timeoutMsg:
          "Expected hidden chat DOM to keep updating in the background",
      },
    );
...
describe("Chat tab auto title", () => {
  it("auto-renames from task updates unless the user has manually renamed the tab", async () => {
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "Expected tab title to auto-update from task title",
      },
    );
  ...
  it("renames the tab via title agent after the first user message is sent", async () => {
    ...
    await browser.waitUntil(
      ...
      },
      {
        timeout: 10000,
        timeoutMsg:
          "Expected tab title to change from default after title agent completes",
      },
    );

    const autoTitle = await (
      await $(`${sel.convTabActive} ${sel.convTabTitle}`)
    ).getText();
    ...
    await browser.waitUntil(
      async () => {
        return browser.execute(
          (titleSel: string, expected: string) => {
            return Array.from(document.querySelectorAll(titleSel))
              .filter((el) => (el as HTMLElement).offsetParent !== null)
      ...
      },
      {
        timeout: 5000,
        timeoutMsg:
          "Expected agents panel card title to match the auto-generated tab title",
      },
    );
  });
});
...
describe("Chat file links", () => {
  it("opens markdown file links in the file viewer with and without line numbers", async () => {
    ...
    await browser.execute(
      (linePath: string, plainPath: string) => {
        const longMock = Array.from(
          { length: 400 },
          (_, i) => `line ${i + 1}`,
        ).join("\n");
        (window as any).__mockReadFileContentByPath = {
    ...
    const clickAssistantLink = async (label: string): Promise<void> => {
      try {
        await browser.waitUntil(
          async () =>
            browser.execute((targetLabel: string) => {
              const links = Array.from(
                document.querySelectorAll(".assistant-message a"),
          ...
          {
            timeout: 5000,
            timeoutMsg: `Expected assistant link to render: ${label}`,
          },
        );
      } catch (_err) {
        const debug = await browser.execute(() => {
          const messages = Array.from(
            document.querySelectorAll(".assistant-message .message-content"),
          ).map((node) => ({
        ...
        throw new Error(
          `Expected assistant link to render: ${label}. Debug: ${JSON.stringify(debug)}`,
        );
      }
      ...
      await browser.execute((targetLabel: string) => {
        ...
        if (!link)
          throw new Error(`Could not find assistant link: ${targetLabel}`);
        link.click();
      }, label);
    };
    ...
    await browser.waitUntil(
      async () =>
        browser.execute(
          (activeSel: string, titleSel: string) => {
            const active = document.querySelector(
              activeSel,
            ) as HTMLElement | null;
        ...
        ),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected file tab agent_mcp_http.rs to open from chat link",
      },
    );
    ...
    await browser.waitUntil(
      async () =>
        browser.execute(() => {
          const nums = Array.from(
            document.querySelectorAll(
              ".diff-panel-file-line .diff-panel-linenum",
            ),
          );
        ...
        }),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected linked line number to be visible in the file viewer",
      },
    );
    ...
    await browser.execute((tabSel: string) => {
      ...
      const chatTab = tabs.find(
        (tab) => !tab.classList.contains("conv-tab-file"),
      );
      if (!chatTab)
        throw new Error("Could not find chat tab to return to message view");
      chatTab.click();
    }, sel.convTab);
    ...
    await browser.waitUntil(
        ...
        ),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected settings.ts file tab to open from chat link without line",
      },
    );
      ...
      async () =>
        browser.execute(() => {
          const lines = Array.from(
            document.querySelectorAll(
              ".diff-panel-file-line .diff-panel-line-text",
            ),
          );
          return lines.some((node) =>
            (node.textContent ?? "").includes("settings value from mock"),
          );
        }),
      {
        timeout: 5000,
        timeoutMsg:
          "Expected file content to render after opening link without line",
      },
    );

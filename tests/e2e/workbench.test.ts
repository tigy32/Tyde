...
describe('Workbench (git worktree)', () => {
  it('comprehensive workbench flow: create via context menu, switch, rename, remove', async () => {
    ...
    const parentEl = await browser.execute((projItemSel, homeItemSel, projNameSel) => {
    ...
    await browser.waitUntil(
      ...
      { timeout: 5_000, timeoutMsg: 'App title did not update to workbench name' },
    );
    ...
    const projectNames = await browser.execute((cardSel, nameSel) => {
      return Array.from(document.querySelectorAll(`${cardSel} ${nameSel}`))
        .map((el) => el.textContent ?? '');
    }, sel.projectCard, sel.projectName);
    ...
    await browser.waitUntil(async () => {
      const name = await browser.execute((wbSel, nameSel) => {
        const item = document.querySelector(wbSel);
